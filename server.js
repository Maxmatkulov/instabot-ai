// ============================================
//  InstaBot AI — v5.1
//  Faqat owner (@mmn0300) Mini App ko'radi
// ============================================
...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  const param = (match[1] || '').trim().replace(/\s/g, '');

  // === OWNER — faqat siz ===
  if (username === OWNER_USERNAME.toLowerCase()) {
    await bot.sendMessage(chatId,
      `👋 Salom, *${name}*! 🔐\n\n🤖 *InstaBot AI* — Admin panel\n\nBarcha sozlamalar quyidagi ilovada 👇`,
      {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [[
  ...
  // === ODDIY FOYDALANUVCHI ===
  await bot.sendMessage(chatId,
    `👋 Salom, *${name}*!\n\n📚 Foydali kontent va darsliklar uchun kanalimizga obuna bo'ling!\n\n👇`,
    {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [[
          ...
          { text: '🎥 Video darsliklar', url: `https://t.me/${TG_CHANNEL.replace('@', '')}` }
        ]]
      }
    }
  );
...
bot.on('callback_query', async (query) => {
  ...
  if (query.data === 'stats' && username === OWNER_USERNAME.toLowerCase()) {
    await bot.answerCallbackQuery(query.id);
    await bot.sendMessage(chatId,
      `📊 *Statistika*\n\n` +
      `⚡ Qoidalar: ${rules.length} ta\n` +
      `⏳ Kutayotganlar: ${pendingIG.size} ta\n` +
  ...
  if (query.data === 'rules' && username === OWNER_USERNAME.toLowerCase()) {
    await bot.answerCallbackQuery(query.id);
    const text = rules.map((r, i) =>
      `*${i+1}. ${r.name}*\nTrigger: \`${r.keywords.join(', ')}\``
    ).join('\n\n');
...
async function sendInstaDM(userId, message) {
  ...
  if (!token || !igUserId) { console.log('⚠️ IG token yo\'q'); return; }
  try {
...
app.get('/test/sub/:userId', async (req, res) => {
  const result = await checkSub(req.params.userId, TG_CHANNEL);
  res.json({ userId: req.params.userId, subscribed: result, message: result ? '✅ Obuna bor' : '❌ Obuna yo\'q' });
});

app.get('/api/rules', (req, res) => res.json(rules));
app.post('/api/rules', (req, res) => {
  const { name, keywords, tgLink, dmFirst, dmSuccess, dmFail } = req.body;
  if (!name || !keywords) return res.status(400).json({ error: 'name va keywords kerak' });
  const rule = { id: Date.now(), name, keywords, tgChannel: TG_CHANNEL, tgLink: tgLink || rules[0].tgLink, dmFirst: dmFirst || rules[0].dmFirst, dmSuccess: dmSuccess || rules[0].dmSuccess, dmFail: dmFail || rules[0].dmFail };
  rules.push(rule);
  res.json(rule);
});
app.delete('/api/rules/:id', (req, res) => { rules = rules.filter(r => r.id != req.params.id); res.json({ ok: true }); });
app.get('/api/stats', (req, res) => res.json({ rules: rules.length, pendingIG: pendingIG.size, igConnected: !!process.env.IG_ACCESS_TOKEN, tgChannel: TG_CHANNEL }));
